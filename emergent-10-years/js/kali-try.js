...
//
  ...
  {left:"L",right:"FO",action:"DIST7", named:"L-FO", t1:"FOX",t2:"D",t3:"L",t4:"L",blocks:["FO-L"]}, // like L-FOX
//
  {left:"A",right:"FOE",action:"DIST1", named:"A-FOE", t1:"FOE",t2:"FOE",t3:"A",t4:"A",blocks:["FOE-A"]},
  {left:"A",right:"FOX",action:"DIST1", named:"A-FOX", t1:"FOX",t2:"FOX",t3:"A",t4:"A",blocks:["FOX-A"]},
...
//
  ...
  {left:"T",right:"FOX",action:"termin", named:"T-FOX"}, // added termin 
  {left:"T",right:"D",action:"termin", named:"T-D"}, // added termin
...
function findTransform(n1) {
  ...
  for (var i=0; i<transformList.length; i++) {
    ...
    if (trans.left == n2type && trans.right == n1.type) {
      switch (trans.action) {
        case "GAMMA-GAMMA": case "DELTA-DELTA": case "beta": case "GAMMA-DELTA": case "DIST0": case "DIST1": case "DIST2":  case "DIST3": case "DIST4": case "DIST5": case "DIST6": case "DIST7": case "termsplit": case "term": case "term3":
...
// added priority to eta rewrites
    var priority = 0;

    if (etaPriority == 1) {
      for (var iprior=0; iprior<transformCache.length; iprior++) {
        if (priority == 0) {
          var transPrior = transformCache[iprior].trans;
          if (transPrior == "eta") {
            var choice = iprior;
            priority = 1;
          }
